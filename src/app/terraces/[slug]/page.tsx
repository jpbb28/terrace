...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const typeStr =
    terrace.terraceType?.map((tt) => TYPE_LABELS[tt] ?? tt).join(" and ") ??
    "terrace";
  const desc =
    `${terrace.name} is a ${typeStr} terrace in ${terrace.neighborhood}, Montréal. ${terrace.description}`.slice(
      0,
      160,
    );
  ...
  return {
    ...
    openGraph: {
      ...
      ...(terrace.photos[0] && {
        images: [
          {
            url: `https://terrasseseason.com${terrace.photos[0]}`,
            alt: terrace.name,
          },
        ],
      }),
...
function buildJsonLd(terrace: Terrace, slug: string) {
  ...
  const establishment = {
    ...
    ...(terrace.photos[0] && {
      image: `https://terrasseseason.com${terrace.photos[0]}`,
    }),
...
export default async function TerracePage({ params }: Props) {
  ...
  const typeStr = terrace.terraceType
    ?.map((tt) => TYPE_LABELS[tt] ?? tt)
    .join(" & ");
  const subtitle = [
    typeStr ? `${typeStr} terrace` : "Terrace",
    terrace.cuisineType,
